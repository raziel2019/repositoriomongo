...
<div class="panel-header colorut" >
		<div class="page-inner py-5">
			<div class="d-flex align-items-left align-items-md-center flex-column flex-md-row">
			    <div>
			        <h2 class="text-black pb-2 fw-bold">Historial Articulos de Revista</h2>
		        </div>		
	        </div>
		</div>
    </div>
<div class="page-inner mt--5">
             ...
             <h4>Buscar Por Año y Periodo</h4>	
               ...
               <form action="/filtroRevista"  method="GET"  style="display: inline-block;"  class="form-group  col-md-5" >
      ...
      <a class="btn btn-primary mb-1 " style="background: #69bb85!important; border-color:#69bb85!important;" href="{{route('revista.create')}}">Registrar Revista</a>
    
      @foreach ($file as $key=>$data)
      <form action="{{ route('revista.select', $data->id) }}"  method="get" style="display: inline;">
      @endforeach
      @csrf
      ...
      <table class="table table-striped table-bordered table-hover">
              ...
              <th>Descargar</th>
                ...
                <!-- Checkboxes -->
                    ...
                    <td>{{++$key}}</td>
                    <td>{{$data->titulo}}</td>
                    <td><a href="{{action('PdfController@pdfRevista', $data->file)}}">{{$data->file}}</a></td>
                    <td>{{$data->cuatrimestre}}</td>
                    <td>{{$data->created_at}}</td>
                    <td><a class="btn btn-success" style="background: #69bb85!important; border-color:#69bb85!important;" href="{{action('PdfController@revista_show', $data->id)}}">Ver </a></td>
                    <td><a class="btn btn-warning" style="background: #69bb85!important; border-color:#69bb85!important;" href="{{action('PdfController@revista_edit', $data->id)}}">Modificar</a> </td>
                    <td>
                        <form action="{{route('revista.destroy', $data->id)}}"  method="POST"> 
                        @method("DELETE")
                        @csrf
                       <input type="submit" class="btn btn-danger" style="background: #69bb85!important; border-color:#69bb85!important;" value="Eliminar">
                       </form>
                    </td>
                    <td><a class="btn btn-default" style="background: #69bb85!important; border-color:#69bb85!important;" href="{{action('PdfController@pdfFormRevista', $data->id)}}">Descargar </a> </td>
