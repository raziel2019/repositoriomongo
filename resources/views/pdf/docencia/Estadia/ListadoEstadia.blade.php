...
<div class="panel-header colorut" >
		<div class="page-inner py-5">
			<div class="d-flex align-items-left align-items-md-center flex-column flex-md-row">
			    <div>
			        <h2 class="text-black pb-2 fw-bold">Historial de Estadia Empresas</h2>
		        </div>		
	        </div>
		</div>
    </div>
<div class="page-inner mt--5">
             ...
             <h4>Buscar Por Año y Periodo</h4>	
               ...
               <form action="/filtroEstadia"  method="GET"  style="display: inline-block;"  class="form-group  col-md-5" >
      ...
      <a class="btn btn-primary mb-1 " style="background: #69bb85!important; border-color:#69bb85!important;" href="{{route('estadia.create')}}">Registrar Estadia </a>
      @foreach ($file as $key=>$data)
      <form action="{{ route('estadia.select', $data->id) }}"  method="get" style="display: inline;">
      @endforeach
      @csrf
      ...
      <table class="table table-striped table-bordered table-hover">
              ...
              <th>Descargar</th>
                ...
                <!-- Checkboxes -->
                    ...
                    <td>{{++$key}}</td>
                    <td>{{$data->nombre}}</td>
                    <td><a href="{{action('PdfController@pdfEstadia', $data->file)}}">{{$data->file}}</a></td>
                    <td>{{$data->cuatrimestre}}</td>
                    <td>{{$data->created_at}}</td>
                    <td><a class="btn btn-success" style="background: #69bb85!important; border-color:#69bb85!important;" href="{{action('PdfController@estadia_show', $data->id)}}">Ver </a></td>
                    <td><a class="btn btn-warning" style="background: #69bb85!important; border-color:#69bb85!important;" href="{{action('PdfController@estadia_edit', $data->id)}}">Modificar</a> </td>
                    <td>
                        <form action="{{route('estadia.destroy', $data->id)}}"  method="POST"> 
                        @method("DELETE")
                        @csrf
                       <input type="submit" class="btn btn-danger" style="background: #69bb85!important; border-color:#69bb85!important;" value="Eliminar">
                       </form>
                    </td>
                    <td><a class="btn btn-default" style="background: #69bb85!important; border-color:#69bb85!important;" href="{{action('PdfController@pdfFormEstadia', $data->id)}}">Descargar </a> </td>
